...
from .shapes import TensorShape,TensorShape_2,TensorShape_3
...
class Graph(object):
    ...
    def compute_output_shapes(self):
        ...
        for node in sorted_nodes:
            if node.kind.decode("utf-8") == "Flatten":
                node.output_shape = TensorShape_2(*NodeKind.compute_output_shape(node))
            elif node.kind.decode("utf-8") == "PriorBox" or  node.kind.decode("utf-8") == "Reshape":
                node.output_shape = TensorShape_3(*NodeKind.compute_output_shape(node))
            elif node.kind.decode("utf-8") == "Concat" or node.kind.decode("utf-8") == "Softmax" or node.kind.decode("utf-8") == "DetectionOutput":
                if len(node.parents[0].output_shape) == 2:
                    node.output_shape = TensorShape_2(*NodeKind.compute_output_shape(node))
                elif len(node.parents[0].output_shape) == 3:
                    node.output_shape = TensorShape_3(*NodeKind.compute_output_shape(node))
            else:
